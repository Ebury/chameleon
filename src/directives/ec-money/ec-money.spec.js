...
describe('EcMoney', () => {
  ...
  it('should add decimals on the last two characters with the defaults values', () => {
    const { input } = mountTemplate('<input v-ec-money data-test="amount-input" />');
  ...
  it('should destroy the directive and unbind the methods', () => {
    const { wrapper, input } = mountTemplate('<input v-ec-money data-test="amount-input" />');
  ...
  it('should throw an error if the directive is not attached to an input', () => {
    withMockedConsole((errorSpy) => {
      expect(() => mountTemplate('<div v-ec-money="{}"></div>')).toThrowError(new TypeError('v-ec-money requires 1 input'));
      expect(errorSpy).toHaveBeenCalledTimes(2);
      expect(errorSpy.mock.calls[0][0]).toContain('v-ec-money requires 1 input');
    });
  });

  it('should ignore letters and only keep the numbers', () => {
    const { input } = mountTemplate('<input v-ec-money data-test="amount-input" />');
    setValue(input, '222222');
    expect(input.element.value).toBe('222,222');
...
function setValue(input, value) {
  ...
  moveCursorToPosition(input, value.length); // move the cursor to the end (simulate the behaviour from the browser. JSDOM puts it at the start of the input)
