...
function useCssResourceAddonSync({
  window,
  document,
}) {
...
export const all = () => ({
  setup() {
    ...
    const { onChanged } = useCssResourceAddonSync({
      window,
      document,
    });
    onChanged(() => {
    ...
    function getInfo(variable) {
      const hslValue = window.getComputedStyle(document.documentElement)
        .getPropertyValue(variable);
      if (hslValue) {
        const color = new Color(`hsl(${hslValue})`);
        return `${color.hsl()
          .string()}, ${color.hex()}, ${color.rgb()}`;
      }
  ...
  template: `
    ...
    <table class="tw-mb-12 tw-w-full tw-border tw-border-solid tw-border-gray-6 tw-font-mono">
      ...
      <tr v-for="i in 7">
        <td class="tw-w-1/6"
            :style="{ backgroundColor: 'hsl(var(--ec-key-color-level-' + i + '))' }"
        ></td>
    ...
    <table class="tw-mb-12 tw-w-full tw-border tw-border-solid tw-border-gray-6 tw-font-mono">
      ...
      <tr v-for="(i, index) in 9">
        <td class="tw-w-1/6"
            :style="{ backgroundColor: 'hsl(var(--ec-gray-color-level-' + index + '))' }"
        ></td>
        <td class="tw-w-2/6 tw-p-4 tw-border-b tw-border-solid tw-border-gray-6">
    ...
    <table class="tw-mb-12 tw-w-full tw-border tw-border-solid tw-border-gray-6 tw-font-mono">
      ...
      <tr v-for="i in 2">
        <td class="tw-w-1/6"
            :style="{ backgroundColor: 'hsl(var(--ec-complementary-color-level-' + i + '))' }"
        ></td>
        <td class="tw-w-2/6 tw-p-4 tw-border-b tw-border-solid tw-border-gray-6">
    ...
    <table class="tw-mb-12 tw-w-full tw-border tw-border-solid tw-border-gray-6 tw-font-mono">
      ...
      <tr v-for="color in ['error', 'error-dark', 'error-light', 'info', 'info-dark', 'info-light', 'success', 'success-dark', 'success-light', 'warning', 'warning-dark', 'warning-light']">
        <td class="tw-w-1/6"
            :style="{ backgroundColor: 'hsl(var(--ec-reserved-color-' + color + '))' }"
        ></td>
        <td class="tw-w-2/6 tw-p-4 tw-border-b tw-border-solid tw-border-gray-6">
    ...
    <table class="tw-mb-12 tw-w-full tw-border tw-border-solid tw-border-gray-6 tw-font-mono">
      ...
      <tr v-for="i in [18, 51, 64, 140, 166, 215, 266, 280, 297, 325]">
        <td class="tw-w-1/6"
            :style="{ backgroundColor: 'hsl(var(--ec-additional-color-level-' + i + '))' }"
        ></td>
        <td class="tw-w-2/6 tw-p-4 tw-border-b tw-border-solid tw-border-gray-6">
    ...
    <div class="tw-bg-gradient-1 tw-w-1/2 tw-flex tw-items-center tw-justify-center tw-resize tw-overflow-auto"
