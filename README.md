...
All utility classes are automatically purged if they are unused anywhere in the `*.vue` or in `*.story.js` files. We are following the guide about [controlling the file size](https://tailwindcss.com/docs/controlling-file-size/) when using the TailwindCSS.
